...
// Phase: ready → play → (hit) death → play+invuln or gameOver when lives=0.
...
export function createRocketRide(engine: GameEngine) {
  ...
  type Phase = 'ready' | 'play' | 'death';
  let phase: Phase = 'ready';
  let readyElapsed = 0; // ms in ready
  let readyGoPlayed = false;
  let lives = 3;
  let invulnTimer = 0; // seconds
  let deathFreeze = 0; // ms brief freeze after hit
  let altitudeFlash = 0; // seconds
  let lastMilestoneAt = 0; // distance meters
  ...
  function clearNearbyObstacles() {
    ...
    for (let i = obstacles.length - 1; i >= 0; i--) {
      const o = obstacles[i];
      if (Math.hypot(o.x - player.x, o.y - player.y) < clearR) {
  ...
  function readyLabel(): string {
    // ~2.5s total: 3 / 2 / 1 / GO
    if (readyElapsed < 550) return '3';
    if (readyElapsed < 1100) return '2';
    if (readyElapsed < 1650) return '1';
    return 'GO';
  }
  ...
  engine.onUpdate((dt) => {
    try {
      ...
      if (deathFreeze > 0) {
        deathFreeze -= dt;
        // still tick invuln/visual timers lightly
        invulnTimer = Math.max(0, invulnTimer - dt / 1000);
        return;
      }
      ...
      // --- READY phase: freeze spawn/collision/movement; draw continues ---
      if (phase === 'ready') {
        readyElapsed += dt;
        if (readyElapsed >= 1650 && !readyGoPlayed) {
          readyGoPlayed = true;
          engine.sound.play('powerup');
        }
        // gentle idle bob on rocket only
        player.y = PLAYER_Y0 + Math.sin(engine.time / 380) * 5;
        flamePhase += sDt * 10;
        // parallax stars drift slowly for life
      ...
      altitudeFlash = Math.max(0, altitudeFlash - sDt);
      invulnTimer = Math.max(0, invulnTimer - sDt);
      flamePhase += sDt * (boostTimer > 0 ? 20 : 14);
      player.squash += (1 - player.squash) * Math.min(1, sDt * 10);

      // A / Space / Z = boost burst
      ...
      // Drag / touch aim — use flat mouseDown/mouseX/mouseY (NOT inp.mouse.down)
      ...
      // Altitude milestones every 500m
      while (lastMilestoneAt + 500 <= distance) {
        lastMilestoneAt += 500;
        engine.addScore(50);
        engine.sound.play('combo');
        altitudeFlash = 1.2;
        engine.spawnParticle(player.x, player.y - 30, '#fbbf24', 16);
        engine.feedback('medium', player.x, player.y - 20, '#fbbf24');
      }
      ...
      for (let i = obstacles.length - 1; i >= 0; i--) {
        const o = obstacles[i];
        o.y += scroll;
        // Telegraph: pulse faster/stronger as obstacle approaches player Y
        const distY = Math.max(0, player.y - o.y);
        const approach = 1 + Math.max(0, 1 - distY / 280) * 2.2;
        o.pulse += sDt * 3 * approach;
        if (o.y > H + 60) {
          obstacles.splice(i, 1);
          continue;
        }
      ...
      // Never let a thrown error kill the RAF loop
      console.error('[rocket-ride] update error', err);
    }
  });
  ...
  engine.onRender(() => {
    try {
      ...
      if (lanePulse > 0 || engine.time < 4000 || phase === 'ready') {
      ...
      for (const o of obstacles) {
        ...
        const distY = Math.max(0, player.y - o.y);
        const warn = Math.max(0, 1 - distY / 260);
        const wob = Math.sin(o.pulse) * (1.5 + warn * 3.5);
        const warnScale = 1 + warn * 0.08 * Math.sin(o.pulse * 2);
        ctx.scale(warnScale, warnScale);
        if (warn > 0.35) {
          ctx.shadowColor = 'rgba(239,68,68,' + (0.25 + warn * 0.55) + ')';
          ctx.shadowBlur = 8 + warn * 18;
        }
        if (o.type === 'fud') {
      ...
      // Rocket (flash while invulnerable)
      const flashHide = invulnTimer > 0 && Math.floor(invulnTimer * 12) % 2 === 0;
      if (!flashHide) {
        ctx.save();
        ctx.translate(player.x, player.y);
        ctx.rotate(player.tilt);
        ctx.scale(2 - player.squash, player.squash);
        if (invulnTimer > 0) ctx.globalAlpha = 0.55 + 0.45 * Math.sin(invulnTimer * 20);
        ...
        const flameH =
          10 +
          (Math.sin(flamePhase) * 0.5 + 0.5) * 14 +
          (boostTimer > 0 ? 10 : 0) -
          (brakeTimer > 0 ? 6 : 0);
      ...
      if (altitudeFlash > 0) {
        const a = Math.min(1, altitudeFlash * 1.2);
        ctx.fillStyle = `rgba(251, 191, 36, ${a})`;
        ctx.font = 'bold 22px monospace';
        ctx.textAlign = 'center';
        ctx.fillText('ALTITUDE BONUS', W / 2, H * 0.28);
        ctx.font = 'bold 14px monospace';
        ctx.fillStyle = `rgba(253, 224, 71, ${a})`;
        ctx.fillText('+50', W / 2, H * 0.28 + 24);
      }
      ...
      if (phase === 'ready') {
        ...
        const isGo = label === 'GO';
        ctx.fillStyle = isGo ? '#4ade80' : '#f8fafc';
        ctx.font = `bold ${isGo ? 56 : 64}px monospace`;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.shadowColor = isGo ? '#22c55e' : '#38bdf8';
        ctx.shadowBlur = 24;
        ctx.fillText(label, W / 2, H * 0.38);
        ctx.shadowBlur = 0;
        ctx.fillStyle = 'rgba(255,255,255,0.7)';
        ctx.font = 'bold 16px monospace';
        ctx.fillText('GET READY', W / 2, H * 0.38 + 52);
      }

      // Lives as 3 small rocket icons top-center
      const lifeIconY = 22;
      const lifeSpacing = 28;
      const livesStartX = W / 2 - lifeSpacing;
      for (let i = 0; i < 3; i++) {
        const lx = livesStartX + i * lifeSpacing;
        ctx.save();
        ctx.translate(lx, lifeIconY);
        ctx.scale(0.45, 0.45);
        if (i < lives) {
      ...
      // Controls hint — keep visible early / during ready
      if (engine.time < 4500 || phase === 'ready') {
      ...
      console.error('[rocket-ride] render error', err);
    }
  });
}
